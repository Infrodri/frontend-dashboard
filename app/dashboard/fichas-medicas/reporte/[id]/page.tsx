...
export default async function ReporteFichaMedicaPage({ params }: ReporteFichaMedicaPageProps) {
  ...
  try {
    ficha = await generateReporte(params.id, session.user.token);
  } catch (error: any) {
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <p className="text-red-500">
          Error al generar el reporte: {error.message}. Por favor, intenta de nuevo.
        </p>
        <Link
                ...
                {ficha.operacionesQuirurgicas.map((op, index) => (
                  <tr key={index}>
                    <td className="border p-2">
                      {op.tipoOperacionQuirurgica || "No especificado"}
                    </td>
                    <td className="border p-2">
            ...
            <table className="min-w-full border-collapse border">
              ...
              <tbody>
                {ficha.ginecologiaObstetrica.map((gyn, index) => (
                  <tr key={index}>
                    <td className="border p-2">
                      {gyn.tipoObstetricoGinecologico || "No especificado"}
                    </td>
                    <td className="border p-2">
            ...
            <table className="min-w-full border-collapse border">
              ...
              <tbody>
                {ficha.adicciones.map((adiccion, index) => (
                  <tr key={index}>
                    <td className="border p-2">
                      {adiccion.tipoAdiccion || "No especificado"}
                    </td>
                    <td className="border p-2">
                      {adiccion.frecuencia || "No especificado"}
                    </td>
                  </tr>
                ))}
              </tbody>
